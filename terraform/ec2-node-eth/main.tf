...
resource "null_resource" "ansible_provision_mount_ebs" {
    count = var.run_ansible_mount_ebs? 1 : 0
     depends_on = [module.ec2-node-eth.ansible_provision_id]
    
    # We making ssh connection in order to wait until intance will be actually reachable
    provisioner "remote-exec" {
      connection {
        type = "ssh"
        host = module.ec2-node-eth.instance_elastic_ip
        user = "ubuntu"
    ...
    provisioner "local-exec" {
      ...
      environment = {
        ...
        HOSTS_PATH = "../terraform/${local.module_name}/${module.ec2-node-eth.ansible_hosts_filename}"
        HOST = "'${module.ec2-node-eth.instance_canonical_name}'"
      }
    }
...
output "ansible_hosts_filename" {
  ...
  value = module.ec2-node-eth.ansible_hosts_filename
}

output "instance_ip" {
  description = "Path to Ansible Hosts file"
  value = module.ec2-node-eth.instance_elastic_ip
}